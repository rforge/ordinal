...
##
##  get_clmFrames() {
##      get_clmFullmf() # get_clm.mf
##      ## compute terms-objects
##  }
##
##  get_clmFullfm() {
##      eval("data.frame-call", envir=parent.envir(2))
##  }
##
##  get_clmDesign() {
##      checkContrasts()
...
##
##  get_clmFormulas:
##  match.call -> formula.list (formulas + envir)
##
##  get_clmFrames:
##  match.call + formula.list -> mf + terms.objects
##
##  get_clmDesign:
##  mf + terms.objects + contrasts -> Design (Y, X, wts, off; S, NOM, etc.)
##

#################################
## o Consider storing form.envir as attr(list.of.formulas, "envir")
## - Consider renaming clm.frames.R to clm.design.R(?)
## o Rename get_clmFullmf to get_clm.mf
## - Check if setting list("formula"=eval.parent(...)) in
##   get_clmFormulas is enough to set its name.
