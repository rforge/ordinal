...
\begin{equation*}
  ...
  \frac{\exp(\theta_j - \bm x_1 \bm\beta)}
  {\exp(\theta_j - \bm x_2 \bm\beta)}
...
\begin{table}
  ...
  \begin{tabular}{llllll}
    ...
    \hline
    Distribution & logistic & Normal & Gumbel (min)$^b$ & Gumbel (max)$^b$ &
    Cauchy ($t_1$)$^c$ \\
    ...
    $\exp(\exp(-\eta))$ & $\arctan(\eta)/\pi + 0.5$ \\
    Density ($f = F'$) & $\exp(-\eta) / [1 + \exp(-\eta)]^2$ & $\phi(\eta)$ &
    & $\exp(-\exp(-\eta) - \eta)$ & $1 / \pi(1 + \eta^2)$ \\
    \hline
  \end{tabular}
  \end{center}
  \footnotesize
...
category $j$ given $\bm x_i$. Thus, similar to the proportional odds
...
income <- data.frame(year=year, freq=freq,
                     income=factor(rep(income, 2), ordered=TRUE,
                       levels=income)) 
tab <- xtabs(freq ~ year + income, income)
attr(tab, "class") <- NULL
attr(tab, "call") <- NULL
...
@       
  ...
  Northeast US reproduced in Table~\ref{tab:incomeMcCullagh80}. 
...
sapply(links, function(link) {
  clm(income ~ year, data=income, weights=freq, link=link)$logLik })
@ 
...
\marginpar{Have I got the signs right in the following?}
...
negative Hessian of the log-likelihood function evaluated at the
maximum likelihood estimates. Standard errors can be obtained as the
...
\begin{equation}
  ...
  \bm H = \frac{\partial^2 \ell(\bm\alpha; \bm y)}{\partial
    ...
    \hat{\bm\alpha}} ~.
\end{equation}
The observed Fisher information matrix is then $I(\hat{\bm\alpha}) =
- \bm H$ and the standard errors are given by
\begin{equation}
  \label{eq:8}
  \textup{se}(\hat{\bm\alpha}) =
  \sqrt{\textup{diag}[I(\hat{\bm\alpha})^{-1}]} .
\end{equation}
...
\marginpar{Mention which is better and refer to Efron and Hinkley}
