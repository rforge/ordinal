...
### FIXME: Make clmm work with zero-weights as clm does.
{
...
### FIXME: Possibly call clm() when there are no random effects?
  ...
  ths <- makeThresholds(frames$ylevels, threshold)
...
### nalpha, nbeta, nrandom, ntau
  ...
  if(use.ssr) {
    rho.clm2clmm.ssr(rho=rho, grList=frames$grList,
                     ctrl=control$ctrl)
### FIXME: call set.AGQ AFTER modification of rho and include AGQ-specific tests in
### set.AGQ. Simply 'return(invisible())' if ssr=FALSE.
    set.AGQ(rho, nAGQ)
  }
  else
...
### contrasts - contrasts for the fixed model terms
  ...
  ## environment (it could be a character), then use the parent frame.
  form.envir <-
...
(number of observations)", length(off), NROW(y)))
  ...
  res <- list(y = y, ylevels=ylevels, X = X, Zt = Zt, wts = as.double(wts),
              off = as.double(off), mf = fullmf, terms = fixedTerms,
              grList = grList, formula = form)
  ## Note: X is with dimnames and an intercept is garanteed.
  return(res)
}
