...
\title{A Primer on Cumulative Link Mixed Models with the \textsf{ordinal}
  Package} 
...
We can fit and summarize this model with the \texttt{clmm} function in
...
quadrature method. Here we use 10 quadrature nodes:
<<>>=
